...
from .executor import ConsumerWithQueue, WorkerPool
...
class SimpleJob(Job):
    ...
    def start(self) -> Any:
        ...
        self.tries += 1
        logging.info(msg=f"{self.name} Called {self.tries} times")
        # one can just omit the return value
        self.f(random.randint(0, 5))
        logging.info(
            msg=f"{self.name} needs {self.retries - self.tries} more calls"
        )
